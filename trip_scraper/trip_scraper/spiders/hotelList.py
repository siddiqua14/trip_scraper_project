    ...
    def parse(self, response):
        ...
        if match:
            try:
                ...
                # Extract inboundCities and select the first city
                ...
                # Select the first city and get its ID and name
                first_city = inbound_cities[0]
                city_id = first_city.get("id")
                city_name = first_city.get("name")
                if not city_id:
                    self.logger.error("City ID not found in the first city!")
                    return
